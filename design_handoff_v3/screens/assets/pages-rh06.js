...
function rhServices(h,o={}){
 ...
 const seedRow=h.seed||unknown||stale
  ?row('Seed','Makes sure the recovery actor exists. Never updated by Quasar.',vers(h.seed||'0.5.0',h.seedOwner==='hand'?'started by the one-line command':'in your stack manager'),own(h.seedOwner==='hand'?'You (docker run)':'External manager'),o.seedUnknown?chip('unknown'):run)
...
function pageRhInventory(variant){
 const h=variant==='gpu'||variant==='unknown'||variant==='error'?rhHost('gpu-host-2'):rhHost('living-room-pc');
 if(variant==='combined')return `<div class="page">${rhDetailHead(h)}${rhServices(h,{agentSub:''})}${rhTrailer}</div>`;
 if(variant==='gpu'){
  const g={...h,seed:'0.5.0',seedOwner:'hand',agent:'0.5.1'};
  return `<div class="page">${rhDetailHead(g)}${rhServices(g,{agentSub:'older than the control plane · update from Releases'})}${rhTrailer}</div>`;}
 if(variant==='unknown')return `<div class="page">${rhDetailHead(h)}${rhServices(h,{unknown:true,foot:false,headHint:'Versions and owners appear once this machine’s recovery actor reports.',
   note:`<div class="note">gpu-host-2 enrolled 20 seconds ago and has not reported its services yet.</div>`})}</div>`;
 return `<div class="page">${rhDetailHead(h)}${rhServices({...h,seed:'0.5.0',seedOwner:'hand'},{stale:true,foot:false,headHint:'Last report from 13:48. Nothing here is acted on until the recovery actor answers again.',
...
function pageRhHosts(){
 ...
 ${head('Fleet',`${online} of ${RH_HOSTS.length} hosts online · 4 sessions running`,
...
function pageRhWarning(variant){
 if(variant==='seed'){
  ...
  <div class="note warn" style="margin-bottom:var(--s4)"><strong>No seed found on gpu-host-2.</strong> Quasar is running normally, but if this machine’s recovery actor is ever deleted, nothing will re-create it. Start the seed again the way you first started it — in your stack manager, or with the command from Add host. The machine keeps its identity and nothing else changes.</div>
  ${rhServices(h,{})}</div>`;}
 if(variant==='seed-unknown'){
  const h={...rhHost('gpu-host-2'),seed:'0.5.0'};
  return `<div class="page">${rhDetailHead(h)}
  <div class="note" style="margin-bottom:var(--s4)"><strong>Seed not checked yet.</strong> The recovery actor on gpu-host-2 looks for the seed every few minutes; the last look was before it restarted. This clears on its own.</div>
...
function pageRhFloor(variant){
 ...
 if(variant==='failed')return `<div class="page">${rhDetailHead(h,'',false)}
  <div class="note warn" style="margin-bottom:var(--s4);display:flex;gap:var(--s5);align-items:center;flex-wrap:wrap"><div style="flex:1;min-width:280px"><strong>The update did not finish; gpu-host-3 still must update.</strong> The new node agent never became healthy, so the recovery actor put v0.4.1 back. Its session ended when the update started. Nothing else changed.${diag('Details','attempt: 1c7e…a4 · outcome: failed, restored\nreason: unhealthy\ncomponents: recovery-actor 0.4.1 → 0.5.2 (succeeded), node-agent 0.4.1 → 0.5.2 (restored)')}</div><button class="btn btn-primary">${icon('refresh')}Try again</button></div>
...
function rhInstalled(o={}){
 ...
 return `<div class="card card-pad"><div class="eyebrow">Installed</div>
  <div style="margin-top:9px">
   ${railFact('Control plane',`<span class="num">v${RH.cp.v}</span>`)}
   ${railFact('Commit',`<a class="mono" style="font-size:var(--t-xs)">${RH.cp.commit}</a>`)}
   ${railFact('Schema',`<span class="num">${RH.cp.schema}</span>`)}
   ${railFact('Database',ext?'Your own':'Quasar’s own')}
   ${railFact('Node agents',`${ext?'4':'5'} hosts · v${RH.cp.v}`)}
  </div>
  ...
  <div style="margin-top:6px">
   ${railFact('Seed',`<span class="num">v0.5.0</span> · external manager`)}
   ${railFact('Recovery actor',`<span class="num">v${RH.cp.v}</span>`)}
   ${ext?railFact('Node agent',muted('none on this machine')):railFact('Node agent',`<span class="num">v${RH.cp.v}</span>`)}
  </div>
...
function rhTargets(){
 return `<div class="card card-pad"><div class="eyebrow">Targets</div>
  ...
  <div style="margin-top:9px">
   ...
   <div class="rel-fact" style="align-items:center"><span>Node agents</span><span class="rowflex" style="gap:8px"><span class="num" style="color:var(--text-2)">3/5</span>${bar(3,5,'warning')}</span></div>
  </div>
...
function rhAddHost(state){
 ...
 const tabsHtml=`<div class="tabs" style="margin:0 0 var(--s5)"><button class="tab${t2?'':' active'}">One-line command</button><button class="tab${t2?' active':''}">Stack manager</button></div>`;
 ...
 if(state==='options'){
  ...
  body=`${tabsHtml}<div class="note warn"><strong>Open this page over HTTPS to add a host.</strong> From an http:// page the command would tell the new host to connect without TLS, sending its enrollment string and host secret across the network in the clear.</div>`;
 }
...
function rhUpdateModal(state){
 ...
 if(state==='own')return modal('Update Quasar',`${lead}
  <div class="note"><strong>Quasar dumps its database first.</strong> Before the control plane on living-room-pc is replaced, its recovery actor dumps Quasar’s database (about 1.4 GB; 212 GB free). If the dump cannot be taken, the update is refused and nothing changes. The last three dumps are kept on that machine.</div>${skip}`,btn(true),540);
 if(state==='own-unknown')return modal('Update Quasar',`${lead}
  <div class="note"><strong>Quasar dumps its database first.</strong> Free space on living-room-pc has not been reported, so it is checked just before the dump. If there is not enough, or the dump fails, the update is refused and nothing changes.</div>${skip}`,btn(true),540);
...
const rhRefusedBanner=`<div class="card card-pad" style="margin-bottom:var(--s4)">
  ...
  <div style="font-family:var(--font-display);font-size:1.25rem;font-weight:600;margin-top:7px">v${RH.cp.v} → v${RH.next.v} did not start</div>
  <p class="hint" style="margin:6px 0 0;line-height:1.55;max-width:80ch;font-size:var(--t-sm)">Quasar could not dump its database on living-room-pc: the disk filled up during the dump. Nothing changed; every target is still on v${RH.cp.v}, and sessions can start again. Free some space on that machine and update again.</p>
  ${diag('Details','attempt: 9e02…7b · control plane · outcome: failed, nothing changed\nreason: backup_failed (no space left on device)\nschema: 88 (unchanged)')}</div>`;

/* ---------------------------------------------------------------------------
   Surface 6 — a failed migrating update and its restore command
   ------------------------------------------------------------------------ */
function rhRestore(state){
 const restoreCmd=`docker run --rm -v /var/run/docker.sock:/var/run/docker.sock ${RH.seedImg} restore --dump 2026-09-25T1402Z-schema-88 --to ${RH.cp.v}`;
 ...
 if(state==='own')body=`${head_}
  ${para(`Quasar does not undo a migrating update on its own, because the new control plane may already have written data. To go back to <b style="color:var(--text)">v${RH.cp.v}</b>, run this on living-room-pc. It stops the control plane, loads the dump taken at 14:02 — before the migration — into Quasar’s database, and starts v${RH.cp.v} again. Anything written after 14:02 is lost.`)}
  ...
  ${para(`Quasar does not undo a migrating update on its own. The recovery actor on living-room-pc has not reported which dump it took, so the restore command cannot be shown here yet. It is printed in that actor’s own output on the machine:`)}
  ${snippet('Run on living-room-pc as root to read it','docker logs quasar-recovery')}
 ...
 const hist=`<div class="card card-pad"><div class="eyebrow">Apply history</div>
  <div style="margin-top:9px"><div class="rel-fact" style="flex-direction:column;gap:3px">
   ...
   <span class="hint" style="font-size:var(--t-xs);color:var(--danger-text)">Failed · not restored · ${state==='external'?'your own database':'dump kept'}</span></div>
...
function rhRemove(state){
 ...
 if(state==='confirm')return modal('Remove gpu-host-2?',
  p(`Quasar drains <b style="color:var(--text)">gpu-host-2</b> — it takes no new sessions and waits for its <b style="color:var(--text)">2 live sessions</b> to end — then stops and removes its node agent and recovery actor.`)+
  p('Its homes and data stay on the machine. The seed stays too, idle; remove it from your stack manager when you like. To bring the machine back, add a host with the same node name: its history is kept.'),
 ...
 const h=rhHost('gpu-host-2');
 if(state==='progress')return `<div class="page">${rhDetailHead({...h,seed:'0.5.0'},chip('removing','info'))}
  <div class="note" style="margin-bottom:var(--s4)"><strong>Removing gpu-host-2.</strong> It takes no new sessions and is waiting for 2 live sessions to end (longest: 41 min so far). Its node agent and recovery actor are removed after that.</div>
  ${rhServices({...h,seed:'0.5.0'},{foot:`<p class="hint" style="margin:0;flex:1">Removal started by salty2011 at 14:12.</p><button class="btn btn-sm">Cancel removal</button>`})}</div>`;
 return `<div class="page">${rhDetailHead({...h,seed:'0.5.0'},chip('drained','warning'))}
  <div class="note warn" style="margin-bottom:var(--s4);display:flex;gap:var(--s5);align-items:center;flex-wrap:wrap"><div style="flex:1;min-width:280px"><strong>Removing gpu-host-2 did not finish.</strong> The host is drained and still enrolled. Its recovery actor could not remove the node agent; nothing else changed.${diag('Details','attempt: 22fd…09 · remove · outcome: failed\nreason: engine error removing quasar-node-agent: device or resource busy')}</div><button class="btn btn-danger btn-sm">${icon('refresh')}Retry removal</button></div>
  ${rhServices({...h,seed:'0.5.0'},{foot:false})}</div>`;
}
...
function rhDevApply(state){
 ...
 const val={
  filled:['ghcr.io/accreleus/quasar-recovery@sha256:9b3e…41d7','ghcr.io/accreleus/quasar-node-agent@sha256:c05a…8e22'],
  empty:['',''],
  error:['registry.example.org/team/quasar-recovery@sha256:9b3e…41d7','ghcr.io/accreleus/quasar-node-agent:my-branch']}[state];
 ...
 <div class="dw-body">
  ...
  <div class="fsec"><div class="fs-label"><h4>Target</h4><p>One machine at a time. Machines built from source are not offered.</p></div>
   <div class="fs-fields"><div class="field"><label class="label">Machine</label><select class="select"><option>gpu-host-2 · GPU host</option><option>living-room-pc · Combined host</option><option disabled>workbench · built from source</option></select></div></div></div>
  <div class="fsec"><div class="fs-label"><h4>Images</h4><p>Leave a service empty to keep what it runs.</p></div>
   <div class="fs-fields">
    ${f('Recovery actor',val[0],err[0],'namespace/quasar-recovery@sha256:…')}
    ${f('Node agent',val[1],err[1],'namespace/quasar-node-agent@sha256:…')}
    ${f('Control plane','','','not on this machine',true)}
   </div></div>
 ...
 <div class="dw-foot"><span class="hint">${ok?'Checks each digest at the registry before anything stops.':state==='empty'?'Enter at least one image by digest.':'Fix the two images above to continue.'}</span>
