...
func certCap(t *testing.T, store *Store, hostID string, gpu int, chainID, rungID string, bitrate int32) (bool, string) {
	...
	must(t, err)
	cert := pickCert(certs, rungID, bitrate, time.Now(), CertStaleness, CertIdentity{})
...
func TestCertsForRungs_batchAcrossChains(t *testing.T) {
	...
	if c := pickCert(certs, "1080p60-h264", 8000, time.Now(), CertStaleness, CertIdentity{}); c == nil || !certShouldCap(*c) {
		t.Error("the 1080p60 rung's own unsafe row must be the one selected")
	}
	if c := pickCert(certs, "720p60-h264", 4000, time.Now(), CertStaleness, CertIdentity{}); c == nil || certShouldCap(*c) {
		t.Error("the 720p60 rung's own ok row must be the one selected")
	}
	if c := pickCert(certs, "1440p60-h264", 8000, time.Now(), CertStaleness, CertIdentity{}); c != nil {
