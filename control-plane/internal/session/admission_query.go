...
func (c candidacy) candidateQuery(policy PlacementPolicy) (string, []any) {
	...
	return `
		...
		WHERE h.status = 'online' AND h.capacity_detection = 'ok' AND h.config_policy_gate_connection IS NULL AND g.reported` + schedulableBindingSQL + pin + image + gate + codec + `
		GROUP BY g.id
...
func (c candidacy) recheckQuery(gpuID string) (string, []any) {
	...
	return `
		SELECT h.status = 'online' AND h.capacity_detection = 'ok' AND h.config_policy_gate_connection IS NULL AND g.reported
...
func (c candidacy) totalsQuery() (string, []any) {
	...
	return `
		SELECT EXISTS (
			...
			WHERE h.status = 'online' AND h.capacity_detection = 'ok' AND h.config_policy_gate_connection IS NULL AND g.reported` + schedulableBindingSQL + pin + image + codec + `
...
func (c candidacy) readinessDiagQuery() (string, []any) {
	...
	return `
		...
		JOIN hosts h ON h.id = g.host_id
		LEFT JOIN sessions s ON s.gpu_id = g.id AND s.state IN ` + activeStatesSQL + `
		WHERE h.status = 'online' AND h.capacity_detection = 'ok' AND h.config_policy_gate_connection IS NULL AND g.reported` + schedulableBindingSQL + pin + image + blocked + codec + `
