...
- **The seed: one container that keeps a machine's recovery actor in existence (#358).**
  ...
  inputs, `docs/configuration.md` "Seed") checks those inputs, then creates exactly one
  recovery actor from its own image; afterwards it re-creates one only if none exists, from
  the last verified actor digest in `seed.json`, and otherwise does nothing. It never
  replaces an actor, writes no machine state, and idles with one log line on invalid inputs,
  an unknown `seed.json` format or an uninstalled marker. `seed.json` format 1, the two labels
  and the compiled actor profile follow ADR 0007 and are pinned by a contract test over
  fixtures per released actor (`testdata/recovery/seed/`, first set added). The actor reads a
  first install's inputs from the seed's container, records `seed.json`, and reports the
  seed's version (new `org.quasar.version` image label) so the host's services card shows it,
  or "not found". The actor and the seed now stop on SIGTERM/SIGINT with exit 0 instead of
  being killed after Docker's timeout.
...
- **Per-GPU codec sets, end to end (#302, amendment 12).** Each GPU reports the codecs it has
  ...
  capacity card show each GPU's codecs as chips beside its slots. At `session_assign` the agent
  refuses a codec outside the bound GPU's current set with `ack{ok:false}` (log token
  `assign-codec-not-in-gpu-set`), a check behind the control plane's own; H.264 is never refused.
...
- **Readiness checks say when and how they were observed, and which ones can block a launch
  (#261).** The agent now reports, per readiness check, when the observation was made and
  where it came from (host probe, local check, container runtime), and marks the
  evidence-based checks — the host probes, the homes storage checks, and the runtime and
  startup-cleanup safety checks — with what they would block if they failed; an
  inconclusive host probe is now reported as `unknown` (shown as indeterminate) instead of
  a warning, and never replaces the last definitive result; the console card shows the
...
- **The host advertises only codecs some usable GPU has proven (#301).** The agent derives
  ...
  built), minus every codec above H.264 without a codec-probe pass on that GPU under the
  current agent image, driver, media settings and GPU identity — and `capacity.codecs`
  becomes the union over GPUs with `encode_slots_total > 0`; a GPU zeroed by a render-node
  pin drops out of it. H.264 is the floor and is always advertised, including when
  GStreamer fails to initialise; HEVC/AV1 need an explicit codec-probe pass, so a freshly
  started host is H.264-only for the tens of seconds its codec probes take to run
  (measured ~19 s on a two-GPU host). `capacity.gpus[].codecs` is not on the wire yet
...
- **Readiness fault harness: the post-recovery launch on NVIDIA no longer races the driver-volume/CUDA-runtime provisioners' self-restart (#292).** `recreate_agent_with_override` and `start_real_agent` now also wait, NVIDIA hosts only, for those provisioners to settle (no pending `*-restart-scheduled` log token, or — if the agent self-restarts — for the new process to register and report `capacity_detection == ok` again) before returning, so a launch placed right after recovery can no longer land in the window that produced a one-off `failed / host_lost` ("host agent connection lost"). Harness-only; no product change.
- A GPU blocked because its media probe decoded the wrong picture now tells the operator the right thing (#287). The readiness remediation says the GPU is working but the picture is wrong, names `QUASAR_ENCODER` only as a way to isolate the fault, and asks for the GPU model, driver version and failing planes, where before it said to check the render node — the one thing that failure had already ruled out. Every other probe failure keeps its existing remediation.
- **A launch right after an agent (re)connects no longer errors instead of retrying (#288).** The web client now auto-retries `503 no_host_available` for up to 20 s, covering the brief window between a reconnecting host registering and its first capacity report landing; the waiting copy no longer claims a slot is being freed when the real cause is a host still coming online, and the control plane now logs every `no_host_available` refusal with the fleet counts behind it (online hosts, hosts awaiting capacity, unreported GPUs, hosts registered in the last 15 s). Admission itself is unchanged.
- **A killed node agent no longer leaks per-session udev export directories (#286).** The
  `{runtime_dir}/udev-<session id>` directory bind-mounted into an app container's
  `/run/udev/data` was reclaimed only by an in-process `Drop`, so a hard agent kill mid-session
  (`host_lost`) left it behind forever — one directory per abnormally ended session. It now
  carries a sibling ownership marker (`udev-<session id>.owner`, written before the directory,
  never inside it) and is retired explicitly at session teardown and, for anything an explicit
  teardown missed, reconciled at boot against this agent's persistent owner token — never a
  container this agent cannot attribute to itself.
- **A killed node agent no longer leaks media-probe runtime directories (#291).** Same leak
  class and fix shape as #286: `{runtime_dir}/quasar-media-probe-<rand>` now carries a sibling
  ownership marker written before the directory, retired explicitly when the probe child exits
  and, for a probe interrupted by a hard kill (a recreate, or the NVIDIA agent's own self-restart
  mid-probe), reconciled at boot against this agent's persistent owner token.
- `make test-rust` is no longer flaky (#285). Node-agent tests no longer change the process environment: each setting a test varies is passed to a pure function, so parallel tests cannot read each other's values and the suite gives the same result whatever the shell exports. Two lock tests now tolerate the moment a sibling test's child process briefly still holds a dropped lock, and the pending-cleanup sweep test runs on its own map. A separate rare hang caused by running out of file descriptors is #290.
- **The `diagnostic_registration` fixture no longer dies from `EMFILE`/`ENFILE`, and a dead fixture is now diagnosed instead of hung out (#290).** Under file-descriptor pressure the in-process fake engine's accept thread used to panic on any accept error other than `WouldBlock`, silently dropping the listener; a test that deliberately tolerates an unreachable engine then waited out its full 20 s bound because a dead fixture looked identical to "still unreachable". `Engine`'s accept loop now treats `EMFILE`/`ENFILE`, and a handler-thread spawn failure, as bounded-retry transients and keeps accepting; a liveness flag and captured cause let a waiting test assert the fixture is alive (and see its real error) before it starts waiting on a bound, instead of after.
- **Host readiness verifies the picture, not just the frame count (#282).** `media_probe_gpu<N>` now decodes its own encoded output with `openh264dec` and scores the decoded planes against the flat clear-colour field the compositor renders when no client is connected. A host whose encode path emits a corrupt picture — the #272 class, which the frame count could not see — now fails the check and stops taking launches. A verification that cannot reach a verdict (no decoder, a decode error, too few frames, the budget spent) is indeterminate, so it neither sets nor clears a block. No new check id, no change to which checks block, no new image dependency.
...
- Dev tooling: the DX test `leakscan:generic-without-operator-patterns` no longer fails on a machine that exports `LEAK_SCAN_REQUIRE_OPERATOR_PATTERNS=1`. It exercises the no-operator-patterns path and expects the generic-only exit, but did not pin that variable, so an operator's standing configuration turned the expected result into the refusal exit and the test failed only for them.
