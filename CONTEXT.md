...
(the GPU has no encoder for that codec) the check reports `unsupported`, a hardware fact
that asks for no attention; any later failure reports `fail`. Its check is
