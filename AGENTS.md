...
| `bench-retro` | `scripts/dx/bench_retro.sh` | replays an archived run manifest (`--manifest FILE`) into the service. The script's default manifest path, `docs/reports/2026-08-16-abr-ladder/bench-retro-manifest.json`, does not exist in this repo — it held the archived runs that predate the service, and did not survive the public-release history squash — so `--manifest` is effectively required here. |
