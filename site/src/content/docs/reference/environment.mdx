...
| `QUASAR_ALLOWED_ORIGINS` | unset | Authoritative override for signaling WebSocket origins. Leave unset for Admin, Settings and upcoming first-run setup to manage the saved list. Explicitly empty still overrides that list. With neither source set, same-origin is allowed. `*` is rejected. |
