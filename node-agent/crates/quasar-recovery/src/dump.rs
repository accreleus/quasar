//! The pre-update dumps in machine state (`CONTEXT.md` "Pre-update dump"): `dumps/` holds
//! each dump as `<name>.dump` beside `<name>.json`, its record. A dump exists once its
//! record does; `pg_dump` writes `<name>.dump.partial` first, which is never read.
//!
