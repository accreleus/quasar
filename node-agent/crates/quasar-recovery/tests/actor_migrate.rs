...
fn a_pre_rh06_dump_is_restored_into_a_fresh_install_before_its_first_boot() {
    ...
    let m = Machine::install(env, true);
    // Postgres runs; no control plane and no agent until the restore.
    ...
    let import = "import-20260925T090000Z";
    let old_stack = FakeDatabase {
        schema_version: 74,
        dirty: false,
    ...
    // Once the control plane has booted, an import is refused.
    std::fs::write(
        m.dir.path().join("dumps").join(format!("{import}.dump")),
        dump_bytes(&old_stack),
    )
    ...
    let refused = m
        ...
        .unwrap_err();
    assert!(
        refused.message.contains("fresh install"),
        "{}",
        refused.message
    );
}
...
#[test]
fn a_dump_newer_than_the_fresh_install_is_refused_before_it_is_loaded() {
    let mut env = combined_env();
    env.insert("QUASAR_AWAIT_RESTORE".into(), "1".into());
    let m = Machine::install(env, true);
    let import = "import-20260925T090000Z";
    ...
    for (bytes, why) in [
        ...
        .unwrap();
        let id = nth_id(why.len() as u64);
        let result = run_restore(&m.actor(), restore_request(&id, Some(import), None));
