...
|---|---|---|---|
| 1 | "Probe GPU/compositor/encoder, audio, uinput, storage permissions/free space and relevant network prerequisites." | **Met** | All four installs report `media_probe_gpu<N>` (composite and encode on the real render node), `application_gpu_probe_gpu<N>`, `audio_probe`, `input_probe`, `homes_root_writable`, `homes_free_space`, `template_free_space` and `media_reachability`, each with observed-at and source: [`install-a/host-body.json`](rh02-265/install-a/host-body.json), [`install-b/host-body.json`](rh02-265/install-b/host-body.json), [`install-c/host-body.json`](rh02-265/install-c/host-body.json). Built in [#257/#259](2026-09-19-rh02-257-259-host-probes-acceptance.md). #280, found on run D (the application GPU probe failing on a healthy host because the probe container was not given the GPU the way a real application container is), was fixed and hardware-validated: see the addendum below and [`fix-validation/280-native-unraid-nvidia-no-override.png`](rh02-265/fix-validation/280-native-unraid-nvidia-no-override.png). **Update 2026-09-20: that limit is closed.** It read: the media probe counts encoded frames and cannot see a corrupt picture, which is how #272 got past it. The probe now decodes its own output and scores the decoded planes against the flat field the compositor renders, so a host whose encode path emits a corrupt picture fails the check and stops taking launches — measured 0.0003 on three healthy configurations across 30 agent restarts against 0.2533 on the reproduced corruption, either side of a 0.01 floor (#282, [record](2026-09-20-282-media-probe-pixel-check-acceptance.md)). #272's cause is removed separately by #281 ([record](2026-09-20-281-linear-encsrc-live-exercise.md)). Both are on branches awaiting the owner; neither is on `develop`. What the new check still cannot see is listed in its own record. |
