...
| 8 | Host-local honesty | none — read every check every host reported during the run | no `id`, `summary` or `remediation` matches (case-insensitive) `browser`, `reachab`, `from the internet`, `externally accessible`, `port forward` … unless the sentence is a negation listed in an explicit allow-list kept in the harness | — | — |
| 9 | Ownership cleanup | after teardown | every `<RID>-*` host row deleted through `DELETE /v1/hosts/{id}` (`204`) with its agent stopped first; zero containers, volumes, networks with the owner label; no volume at all that was not there at preflight (an anonymous volume carries no label); zero `quasar-sess-*` / `quasar-pulse-*` / `quasar-probe-*` containers; `/var/lib/<RID>` gone | — | — |
