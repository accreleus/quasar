...
(GPU-vendor-agnostic Vulkan video encode), per the adoption spec (`docs/research/vulkan-encode-unified-pipeline-adoption-spec.md`,
which was deliberately not carried over to the public repository and is not public in this
repo). Its `build`/`runtime`/`nv`
...
branch — the working notes on the nine PRs open at the time (#42–#50) and which commits they
correspond to were at `docs/reports/2026-07-26-gwd-upstream-submission-drafts.md`, which was deliberately not carried over to the public repository; check the PRs themselves upstream for current status.
