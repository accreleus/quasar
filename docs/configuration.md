...
| `QUASAR_PLATFORM_RELEASE_REPO` | `accreleus/quasar` | **Platform-release detection (#104/#110).** The `owner/name` repository whose GitHub Releases the `platform.release_detect` job reads. Point it at a fork to follow that fork's releases. Set it to the **empty string** to turn detection off entirely: the job still exists in the Jobs page but every run is `skipped` with a reason, no outbound request is made, and the admin Releases page simply reports nothing available. (Unset falls back to the default, which is the normal deployment; explicitly empty is the off switch.) |
...
| `QUASAR_PLATFORM_RELEASE_ASSET_HOSTS` | `github.com,objects.githubusercontent.com` | Comma-separated egress allowlist for **release asset downloads**. The asset URL is remote-supplied (it comes out of the releases listing), so a manifest whose `browser_download_url` names a host outside this list is refused by name and the release is not recorded. Separate from the API host because GitHub serves assets from its own CDN. |
