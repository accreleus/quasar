...
| `QUASAR_INTEL_VULKAN_VIDEO` | **on** | **Intel hosts only; inert on AMD/NVIDIA.** Mesa's Intel Vulkan driver (ANV) hides the entire Vulkan Video extension family behind an opt-in instance debug flag, and with it unset the physical device does not advertise `VK_KHR_video_queue` at all. Every other video extension declares that one as a dependency, so GStreamer's device open logs `Could not enable extension VK_KHR_video_queue` and then registers **no** vulkan video element — while `vulkansink` still appears, because the device itself opened fine. The observable symptom is a host whose codec probe reports an empty set and whose `encoder_codecs` readiness check fails with "has a GPU but the encoder advertises NO codecs" (#126). When the agent detects an Intel GPU it therefore sets `ANV_DEBUG=video-decode,video-encode` before `gstreamer::init`, merging with any value an operator already set rather than replacing it. Set this knob to `0`/`false`/`off` to leave `ANV_DEBUG` untouched; unset, empty, `1`, `true` or `on` all mean enabled, and any other value is ignored with a warning and stays enabled. Three things to know before relying on it. **It only matters when the host's encoder is `vulkan`** — Intel's vendor default is VA (see `QUASAR_ENCODER`), and the codec probe is encoder-specific, so on a default Intel host this changes nothing observable. Mesa ships the flag off by default and does not treat ANV Vulkan Video as validated, so this is an opt-in path rather than a supported one. And ANV gates *encode* on `verx10 < 125`: Gen12.0 integrated graphics (Tiger, Rocket, Alder and Raptor Lake) qualify, while DG2/Arc and newer parts do **not** and get Vulkan *decode* only, still needing VA-API to encode. One operator trap: the agent sets `ANV_DEBUG` inside its own process, so a `docker exec … gst-inspect-1.0 vulkanh264enc` that does not set the variable itself will report the element missing on a host where sessions encode fine. |
