...
(#365), so a new host starts on the control plane's release. Each variable below overrides one
of them:

| Variable (control plane) | Meaning |
|---|---|
| `QUASAR_ENROLL_SEED_IMAGE` | Override of the seed, `<registry>/quasar-recovery@sha256:<digest>`. The seed's first actor is created from it. |
| `QUASAR_ENROLL_AGENT_IMAGE` | Override of the node agent, `<registry>/quasar-node-agent@sha256:<digest>`. |
...
with only a format-1 manifest) has no default, so there only the variables apply. The control
...
seeds. With neither, the dialog says so and creates nothing. A tag in either variable fails
startup: the seed refuses one.
