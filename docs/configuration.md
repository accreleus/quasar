...
| `QUASAR_ENROLL_SEED_IMAGE` | unset | The seed image Admin → Fleet → Add host installs on a new GPU host, as `repository@sha256:<digest>` (the `quasar-recovery` image; a tag fails startup). See "Add host" below. |
| `QUASAR_ENROLL_AGENT_IMAGE` | unset | The node-agent image Add host installs, as `repository@sha256:<digest>` (a tag fails startup). See "Add host" below. |
...
| `QUASAR_INTRA_REFRESH_PERIOD` | `0` (continuous) | **Recommended: `60`** (validated 2026-07-22 — a 1 s wave matched continuous mode's loss recovery — zero hitch windows across clean/steady-loss/burst — while intra-coding a fraction of the data; continuous showed no measurable benefit over it; the experiment write-up, `docs/reports/ir-period-experiment-2026-07-22.html`, was deliberately not carried over to the public repository).  `vulkanh264enc`'s `intra-refresh-period` in frames — frames between the start of one refresh cycle and the next. `0` runs back-to-back cycles continuously. Ignored unless `QUASAR_INTRA_REFRESH=1`. Same Vulkan-only, patched-image-only scope as `QUASAR_INTRA_REFRESH`. |

### Steam preparation (#145; upcoming release)

The Steam source setting **Prepare Steam for faster first launch** controls
background preparation and use of prepared homes together. It defaults to enabled
on supported deployments. It does not change Steam discovery, source enablement,
or permission to launch. Only explicitly supported Steam images are eligible;
HOME/WorkingDir metadata alone does not make another image eligible.

Effective production and consumption each require the Steam source policy and
that host's respective permission below. An absent host flag defaults to enabled;
an explicitly disabled host flag remains disabled across upgrade. Turning the
source setting off wins over both permissions, cancels preparation safely, and
prevents seeding new homes. Existing homes, running user sessions and published
templates are preserved. Re-enable to reconcile missing or stale templates on
already-installed images; no new download is required.

Preparation runs in the background after installation or adoption changes and
yields to user sessions. A missing, failed or disabled template never prevents a
normal Steam launch. New users receive independent writable copies of sanitized
prepared files; existing non-empty homes are not overwritten. Image details report
preparation separately from image download readiness. Older agents that cannot
apply/report the policy need upgrading; the desired switch alone is not evidence
that preparation is active on a host.

Clone mode remains `auto`: the agent probes the actual template/home mount paths,
uses reflinks when supported, and otherwise makes a full copy with the reason
reported. A share layer may lack reflinks even when its underlying native pool
supports them. Choose an appropriate native filesystem bind during deployment if
reflinks are wanted; Quasar does not relocate data or rewrite host mounts.
Changing the configured roots causes capability detection to be refreshed.

The settings below remain advanced per-host controls.


| Variable | Default | Values / notes |
|---|---|---|
| `QUASAR_HOME_TEMPLATES` | enabled when absent | **Consumption permission.** An explicit `0`/`false` disables seeding prepared Steam homes on this host. Enabling this permission cannot override a disabled Steam source preparation policy or make an unsupported image eligible. Existing non-empty homes are always preserved. |
...
| `QUASAR_TEMPLATE_CLONE_MODE` | `auto` | How to seed a managed home from a template: `auto` (probe reflink support; use reflink if available, else copy), `reflink` (require reflink, fail open to cold boot if unsupported), `copy` (always full-copy, no probe), or `off` (cloning disabled, use cold boot). Recommend `auto` for most deployments. A reflink shares data blocks until a home changes; a full copy takes time and space proportional to the prepared files. `off` is the fallback when the root is misconfigured or unavailable. |
| `QUASAR_TEMPLATE_WARMUP` | enabled when absent | **Production permission.** An explicit `0`/`false` disables background template preparation on this host while permitting separately enabled consumption of existing templates. Enabling this permission cannot override a disabled Steam source preparation policy or make an unsupported image eligible. |
...
(`node-agent/src/session/ladder.rs`; the spec, `docs/superpowers/specs/2026-08-16-abr-resolution-fps-ladder-design.md`
§D1/§D2, was deliberately not carried over to the public repository). Take effect when `abr_ladder_resolution` is on — and, because the **fps** rung shares this
...
| `QUASAR_ABR_LADDER_RES_RECOVER_DWELL` | `2` | `1..60` | Consecutive recovered windows required before an up-step. **2026-08-18 flip** (was `4`; T3 `ladder_recover_fast` — time-to-launch-rung 55.2s → 37.6s (−32%), 0 oscillations; the T3 report, `docs/reports/2026-08-18-overnight-optimisation/t3-ladder-step-recovery.md`, was deliberately not carried over to the public repository). |
...
either path — absent from both the compose passthrough and the hostcfg catalog (per the
2026-08-18 T3 report, `docs/reports/2026-08-18-overnight-optimisation/t3-ladder-step-recovery.md`,
...
| `QUASAR_ABR_MAX_UP_STEP` | `0.25` | Max fractional increase per up-step, `>0`. **2026-08-18 flip** (was `0.10`; T3 `abr_up_fast` — post-impairment setpoint ramp 37.8s → 14.0s, client-present drops 12.2% → 1.7%, 0 oscillations; the T3 report, `docs/reports/2026-08-18-overnight-optimisation/t3-ladder-step-recovery.md`, was deliberately not carried over to the public repository). Settable via Host Settings since 2026-08-18. |
...
| `QUASAR_ABR_LADDER_RECOVER_DWELL` | `2` | Consecutive `healthy` windows required before stepping a rung down, `>= 1`. **2026-08-18 flip** (was `4`; T3 `ladder_recover_fast`, alongside `QUASAR_ABR_LADDER_RES_RECOVER_DWELL` below; the T3 report, `docs/reports/2026-08-18-overnight-optimisation/t3-ladder-step-recovery.md`, was deliberately not carried over to the public repository). |
...
**by digest** (a tag is resolved to its registry digest, and an image with none is refused):
...
Arcane. Both install the two images the control plane is configured with:

| Variable (control plane) | Meaning |
|---|---|
| `QUASAR_ENROLL_SEED_IMAGE` | The seed, `<registry>/quasar-recovery@sha256:<digest>`. The seed's first actor is created from it. |
| `QUASAR_ENROLL_AGENT_IMAGE` | The node agent, `<registry>/quasar-node-agent@sha256:<digest>`. |

The control plane writes them into the `/enroll-host.sh` it serves, and the dialog reads them
...
this build it installs a **GPU host** only. The seed creates it (above); it can still be
...
| `QUASAR_ENROLLMENT` | — (**required** on first install) | The `qenr1.…` string from Admin → Fleet → Add host. Stored as a 0600 secret in machine state and handed to the agent as the read-only file `QUASAR_ENROLLMENT_FILE`; never put in the agent's environment. Read only on the first install: once machine state exists the stored secret wins and this value is ignored (logged at INFO), whatever a later start carries. A `docker run` container's environment cannot be edited, so the value stays visible in `docker inspect quasar-recovery` until the actor is recreated without it; the token is single-use and expires, so after enrollment it enrolls nothing. |
...
`scale_stage::vulkan_resize_validated_for_codec` now passes every codec (the validation
write-up, `docs/reports/2026-08-22-vulkanscale-validation/H265-PROFILE-CAPS.md`, was deliberately not carried over to the public repository). On an
...
`web/src/pages/app/hud/panes/DisplayPane.tsx`, default off, because of the KDE/KWin gap
...
`encode_ms` improving at a lower rung; the validation matrix that documented this,
`docs/superpowers/specs/2026-08-16-adaptive-external-resolution-design.md`, was deliberately not carried over to the public repository.
